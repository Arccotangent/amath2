...
Addition::Addition(vector<ex> args) {
	this->args = move(args);
}

Addition &Addition::getInstance(vector<ex> args) {
	static Addition *instance = nullptr;

	if (instance == nullptr) {
		instance = new Addition(move(args));
	}
